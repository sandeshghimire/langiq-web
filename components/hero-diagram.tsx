        ...
        <div style={{ position: "relative" }}>
            ...
            >
                ...
                {TEST_RUNS.map((run, i) => {
                    ...
                    const clr = isPass ? "var(--accent)" : isRun ? "#FFB547" : "var(--text-tertiary)";
                    return (
                        <g key={run.label}>
                            {isRun && <rect x="12" y={y - 10} width="174" height="28" rx="2" fill="rgba(255,181,71,0.06)" />}
                            {isPass && (
                                <>
                            ...
                            {isRun && (
                                <>
                                    <circle cx="28" cy={y + 3} r="7.5" fill="none" stroke="#FFB547" strokeWidth="1" />
                                    <circle cx="28" cy={y + 3} r="3.5" fill="#FFB547" opacity="0.85" />
                                </>
                            )}
                            ...
                            {isRun && <text x="182" y={y + 8} fill="#FFB547" fontSize="8" fontFamily="var(--font-jetbrains, monospace)" letterSpacing="0.07em" textAnchor="end">RUN</text>}
                        </g>
                    );
                })}
                ...
                {LOG_LINES.map((line, i) => {
                    const ly = 346 + i * 17;
                    const clr = line.level.trim() === "PASS" ? "#00D9C0" : "#FFB547";
                    return (
                        <g key={i}>
